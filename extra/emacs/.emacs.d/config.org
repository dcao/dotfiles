...
(use-package org
  ...
  (use-package org-expiry :ensure nil)
...
*** org-brain
#+begin_src emacs-lisp
(defun dcao/capture-add-id ()
  (let ((key  (plist-get org-capture-plist :key))
        (desc (plist-get org-capture-plist :description)))
    (when (and (not org-note-abort) (equal key "b"))
      (org-id-get-create))))

(add-hook 'org-capture-prepare-finalize-hook 'dcao/capture-add-id)

(push '("e" "brain edit" plain (function org-brain-goto-end)
        "* %i%?" :empty-lines 1)
      org-capture-templates)
...
(use-package org-brain
  ...
  :init
  (setq org-brain-path (concat dcao/org-root "brain"))
  ...
  (setq org-brain-title-max-length 60)
